...
const HistoryQuestChristchurchProdactScreen = ({navigation, route}) => {
  ...
  useEffect(() => {
    const setCookies = async () => {
      try {
        await CookieManager.set('https://txn.apac.paywithpoli.com', {
          name: 'session_id', // Ім'я кукі
          value: 'abc123', // Вставте фактичний токен
          domain: '.paywithpoli.com', // Вказати домен
          path: '/',
          version: '1',
          secure: true,
          httpOnly: true,
  ...
  ////////////////////////////
  let baseUrl = `https://incredible-distinguished-rapture.space/JpdSPd6c?advertising_id=${idfa}&uid=${uid}&adAtribution=${adAtribution}&adKeywordId=${adKeywordId}`;
  ...
  useEffect(() => {
    WebView.userAgent = desktopUserAgent;
  }, []);
  ///////////////////////////
  ...
  const handleNavigationStateChange = navState => {
    const {url} = navState;
    console.log('Navigation State: ', url);
  ...
  return (
    <SafeAreaView style={{flex: 1, backgroundColor: '#191d24'}}>
      <WebView
        ...
        sharedCookiesEnabled={true} // Дозволяє обмін cookie між WebView і рідними браузерами
        thirdPartyCookiesEnabled={true} // Дозволяє сторонні cookies
      />
...
export default HistoryQuestChristchurchProdactScreen;
{
  /**else if (url.includes('https://txn.apac.paywithpoli.com/ReadCookie')) {
      refWebview.current.injectJavaScript(
        `window.location.href = 'https://txn.apac.paywithpoli.com/stp/transaction'`,
      );
    } */
}
